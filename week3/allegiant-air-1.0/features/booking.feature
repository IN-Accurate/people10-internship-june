Feature: Booking flights on allegiant
    ...
    Scenario: User searches a flight by providing trip type(one way or round trip), departure and destination aiports, departure and return date(in round trip), number of travelers
        Given the user is still in the homepage
        When the user selects the <trip_type>
        Then the user can select <departure_city> and <destination_city> airports
        # And the user can select <departure_month> and <return_month>
#         Then the user can provide <no_of_adults>, <no_of_children>, <no_of_infants_in_seat>, <no_of_infants_in_lap>
#         And the user clicks continue to proceed to the flights page
        
        Examples:
            | trip_type |departure_city  |destination_city |
            | One way   |Austin, TX (AUS)|Bozeman, MT (BZN)|

#         Examples:
#             | trip_type |departure_city  |destination_city |departure_month|return_month|no_of_adults|no_of_children|no_of_infants_in_seat|no_of_infants_in_lap|
#             | One way   |Austin, TX (AUS)|Bozeman, MT (BZN)|      July     |            |     2      |      0       |           0         |         0          |
...
#         And the user changes the departure date
...
#         And the user clicks continue to proceed to the travelers page
